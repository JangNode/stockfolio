/**
 * (1회성) 종목 시세 원자료 hot/cold 분리 도입 시 stock_daily_prices_recent(Postgres)를
 * 기존 Parquet 백필 데이터로 채운다. 이미 Parquet에 있는 최근 STOCK_DATA_HOT_WINDOW_YEARS
 * 년치를 그대로 복사해 넣는 것뿐이라 새 API 호출은 없다. 이후로는
 * scripts/update-stock-daily-prices-recent.ts(매일)가 이 표를 이어서 채운다.
 *
 * server-only로 막힌 lib/supabaseAdmin.ts를 순수 Node 스크립트에서도 재사용하려면
 * "react-server" 조건으로 실행해야 한다:
 *   tsx --conditions=react-server scripts/seed-stock-daily-prices-recent.ts
 *
 * 필요 환경변수: NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
 */

import { downloadYearPrices, hotWindowStartDate, upsertRecentPrices } from "@/lib/stockDailyPricesStorage";
...
async function main(): Promise<void> {
  ...
  console.log(`시딩 완료: 총 ${total}행을 stock_daily_prices_recent에 저장했습니다.`);
}
