/**
 * 종목 시세/재무/배당 원자료(raw data) 관련 기준값. 특정 전략 전용이 아니라 여러
 * 전략이 같이 쓸 공유 데이터라서 dh_ 접두사 없이 별도 파일로 둔다(2026-08-28,
 * 당시엔 DH전략이 유일한 사용처였으나 이후 dh_value_dividend는 제거됐고
 * peg_lynch 등이 그대로 재사용 중이다). 전략 고유의 스크리닝 기준값(PEG 상한 등)은
 * 각 전략의 *Config.ts에 따로 둔다.
 */

// 후보종목 발굴 기준(시가총액, 억원) — 이 문턱을 한 번이라도 넘은 적 있는 종목만
// 재무/배당 백필 대상으로 삼는다. 대형주 기준과 지금은 같은 값이지만, 다른 전략이
